...
namespace DotNet.MultiSourceConfiguration.Implementation
{
    public static class DefaultConverterFactory
    {
        public static Dictionary<Type, UnifiedConverter> GetDefaultConverters()
        {
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<bool>>(new List<bool>(), s => s.Split(',').Select(Boolean.Parse).ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<int>>(new List<int>(), s => s.Split(',').Select(x => Int32.Parse(x, CultureInfo.InvariantCulture)).ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<long>>(new List<long>(), s => s.Split(',').Select(x => long.Parse(x, CultureInfo.InvariantCulture)).ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<string>>(new List<string>(), s => s.Split(',').ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<double>>(new List<double>(), s => s.Split(',').Select(x => double.Parse(x, CultureInfo.InvariantCulture)).ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<decimal>>(new List<decimal>(), s => s.Split(',').Select(x => decimal.Parse(x, CultureInfo.InvariantCulture)).ToList()));
            ...
            converters.AddTypeConverter(new LambdaConverter<IEnumerable<float>>(new List<float>(), s => s.Split(',').Select(x => float.Parse(x, CultureInfo.InvariantCulture)).ToList()));
            return converters;
        }
